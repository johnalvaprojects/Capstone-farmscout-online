...
/** @type {string[]} */
let mfCarouselImages = [];
/** @type {Array<Record<string, unknown>>} */
let mfCarouselMeta = [];
/** When true, slides are text “box” cards (no images); gradients use per-index fallbacks. */
let mfCarouselUseBoxes = false;
/** @type {((meta: Record<string, unknown>) => void) | null} */
let mfOnCategorySelect = null;
let mfListenersAbort = null;
let mfResizeHandler = null;
...
function waitForImages() {
  if (mfCarouselUseBoxes) return Promise.resolve();
...
/**
 * Parse "5:30 AM - 6:00 PM" hours from description.
 * @param {string} desc
 * @returns {{start:number,end:number}|null}
 */
function parseHoursRange(desc) {
  const s = String(desc || '');
  const m = s.match(/(\d{1,2}(?::\d{2})?\s*(?:AM|PM))\s*-\s*(\d{1,2}(?::\d{2})?\s*(?:AM|PM))/i);
  if (!m) return null;
  const toMin = (t) => {
    const mm = String(t || '').trim().match(/^(\d{1,2})(?::(\d{2}))?\s*(AM|PM)$/i);
    if (!mm) return null;
    let h = Number(mm[1]);
...
/**
 * Stable pseudo-random "updated minutes ago" per card per session.
 * @param {string} key
 * @returns {number}
 */
function getUpdatedMinutes(key) {
  try {
    const k = 'fs_mf_updated_' + String(key || '');
    const raw = sessionStorage.getItem(k);
    if (raw) {
      const ts = Number(raw);
      if (Number.isFinite(ts) && ts > 0) {
        return Math.max(0, Math.min(120, Math.floor((Date.now() - ts) / 60000)));
      }
    }
    // Seed 2–14 minutes ago
    const seeded = Date.now() - (2 + Math.floor(Math.random() * 13)) * 60000;
    sessionStorage.setItem(k, String(seeded));
...
function createCards() {
  ...
  const slideCount = mfCarouselUseBoxes
    ? mfCarouselMeta.length
    : mfCarouselImages.length;
  ...
  for (let i = 0; i < slideCount; i++) {
    ...
    const meta = mfCarouselMeta[i];
    if (meta) {
      const label = meta.filipino_name || meta.category_name || meta.name || 'Category';
      card.setAttribute('title', String(label));
      card.setAttribute('aria-label', String(label));
    }

    if (mfCarouselUseBoxes) {
      const title = meta?.category_name || meta?.name || 'Category';
      const desc = meta?.desc ?? meta?.description ?? '';
      const num = Number(meta?.product_count ?? meta?.count ?? 0);
      const hours = parseHoursRange(String(desc || ''));
      const open = isOpenNow(hours);
      const updated = getUpdatedMinutes(String(title || i));
      const statusText = open ? 'LIVE NOW' : 'CLOSED';
      card.innerHTML = `
        <div class="mf-parallax-card-inner">
          <div class="mf-mk-media" aria-hidden="true">
          ...
          <div class="mf-mk-top">
            <div class="mf-mk-status ${open ? 'is-live' : 'is-closed'}">
              <span class="mf-mk-dot" aria-hidden="true"></span>
              <span>${statusText}</span>
            </div>
            <div class="mf-mk-updated">Updated ${updated} min${updated === 1 ? '' : 's'} ago</div>
          </div>
          <h4 class="mf-parallax-title">${escapeHtml(String(title))}</h4>
          <p class="mf-parallax-desc">${escapeHtml(String(desc))}</p>
          <div class="mf-mk-tags" aria-label="Popular categories">
            <span class="mf-mk-tag">Vegetables</span>
            <span class="mf-mk-tag">Fish</span>
            <span class="mf-mk-tag">Rice</span>
          </div>
          <p class="mf-parallax-count"><strong>${Number.isFinite(num) ? num : 0}</strong> products</p>
          <div class="mf-mk-cta" aria-hidden="true">
            <span>View Market</span>
            <span class="mf-mk-cta-arrow">→</span>
          </div>
        </div>`;
    } else {
      const src = mfCarouselImages[i];
      const img = new Image();
      img.className = 'card__img';
      img.alt = meta ? String(meta.filipino_name || meta.category_name || meta.name || '') : '';
      img.decoding = 'async';
      img.loading = 'eager';
...
function buildPalette() {
  if (mfCarouselUseBoxes) {
    gradPalette = items.map((_, i) => fallbackFromIndex(i));
    return;
  }
...
// Drag state (listeners attached per carousel run)
let dragging = false;
let lastX = 0;
let lastT = 0;
let lastDelta = 0;
...
function attachMfCarouselListeners(signal) {
  if (!stage) return;
  ...
  stage.addEventListener(
    ...
    (e) => {
      ...
      // Tap (no real drag): select the market/category under the pointer, not only whichever card is centered
      if (wasDragging && mfOnCategorySelect && !dragMoved) {
        let idx = activeIndex;
        const target = e.target;
        if (target && items.length) {
          for (let i = 0; i < items.length; i++) {
            if (items[i].el && items[i].el.contains(target)) {
              idx = i;
              break;
            }
          }
        }
        const meta = mfCarouselMeta[idx];
        if (meta) mfOnCategorySelect(meta);
      }
    },
    { signal }
  );
  ...
  window.addEventListener('resize', mfResizeHandler, { signal });

  document.addEventListener(
    'visibilitychange',
    () => {
      if (!document.getElementById('mfCategoryStage')) return;
      if (document.hidden) {
        cancelCarousel();
        cancelBG();
      } else {
...
function resetMfCarouselRuntimeState() {
  cancelCarousel();
  cancelBG();
  items = [];
  positions = [];
...
/**
 * Initialize the carousel application (FarmScout Market Finder categories)
 */
async function init() {
  stage = document.getElementById('mfCategoryStage');
  cardsRoot = document.getElementById('mfCategoryCards');
  bgCanvas = document.getElementById('mfCategoryBg');
  loader = document.getElementById('mfCategoryLoader');
  bgCtx = bgCanvas ? bgCanvas.getContext('2d', { alpha: false }) : null;

  const hasImageSlides = !mfCarouselUseBoxes && mfCarouselImages.length > 0;
  const hasBoxSlides = mfCarouselUseBoxes && mfCarouselMeta.length > 0;
  if (!stage || !cardsRoot || !bgCanvas || (!hasImageSlides && !hasBoxSlides)) {
    return;
  }

  resetMfCarouselRuntimeState();

  // Preload images for faster loading
  if (!mfCarouselUseBoxes) {
    preloadImageLinks(mfCarouselImages);
  }
  ...
  // Warmup GPU compositing (lighter for box slides — still helps first drag)
  ...
  startBG();
  await new Promise((r) => setTimeout(r, mfCarouselUseBoxes ? 16 : 100)); // Let background settle
  ...
  if (mfListenersAbort) {
    attachMfCarouselListeners(mfListenersAbort.signal);
  }
}

// ============================================================================
// FarmScout: start/stop from market-finder.php
// ============================================================================

window.mfCategoryCarouselDestroy = function mfCategoryCarouselDestroy() {
  resetMfCarouselRuntimeState();
  mfCarouselUseBoxes = false;
  if (mfListenersAbort) {
    try {
      mfListenersAbort.abort();
    } catch (_) {}
    mfListenersAbort = null;
  }
  const cr = document.getElementById('mfCategoryCards');
  if (cr) {
    cr.innerHTML = '';
  }
  const ld = document.getElementById('mfCategoryLoader');
  if (ld) {
    ld.classList.remove('loader--hide');
  }
  const st = document.getElementById('mfCategoryStage');
  if (st) {
    st.classList.remove('carousel-mode', 'dragging');
  }
  stage = null;
  cardsRoot = null;
  bgCanvas = null;
 ...
 * @param {Array<Record<string, unknown>>} metaList
 * @param {{ onSelect?: (meta: Record<string, unknown>) => void; useBoxes?: boolean }} [options]
 */
window.mfCategoryCarouselStart = async function mfCategoryCarouselStart(imageUrls, metaList, options) {
  window.mfCategoryCarouselDestroy();
  mfCarouselImages = imageUrls || [];
  mfCarouselMeta = metaList || [];
  mfOnCategorySelect = options && options.onSelect ? options.onSelect : null;
  mfCarouselUseBoxes = !!(options && options.useBoxes);
  mfListenersAbort = new AbortController();
  await init();
};